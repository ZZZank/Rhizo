...
public class UnitStorage {
	...
	public UnitStorage() {
		...
		addFunc("random", RandomUnit.SUPPLIER);
		addFunc("time", TimeUnit.SUPPLIER);
		...
		addFunc("color", a -> ColorUnit.FACTORY.create(a));
	...
	@Nullable
	public Unit createFunc(String name, Unit[] args) {
	...
	public Unit createFunc(String name, List<Unit> args) {
		return createFunc(name, (Unit[]) args.toArray());
	}
