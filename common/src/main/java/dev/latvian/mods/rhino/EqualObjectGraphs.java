...
final class EqualObjectGraphs {
	...
	private boolean equalScriptables(Context cx, final Scriptable s1, final Scriptable s2) {
		...
		for (int i = 0; i < l; ++i) {
			if (!equalGraphs(cx, getValue(cx, s1, ids1[i]), getValue(cx, s2, ids2[i]))) {
				return false;
			}
		}
		...
		if (s1 instanceof NativeContinuation) {
			...
			return s2 instanceof IdFunctionObject && IdFunctionObject.equalObjectGraphs(
				cx,
			...
			return s2 instanceof ArrowFunction && ArrowFunction.equalObjectGraphs(
				cx,
				(ArrowFunction) s1, (ArrowFunction) s2, this);
		} else if (s1 instanceof BoundFunction) {
			return s2 instanceof BoundFunction && BoundFunction.equalObjectGraphs(
				cx,
				(BoundFunction) s1, (BoundFunction) s2, this);
		} else if (s1 instanceof NativeSymbol) {
			return s2 instanceof NativeSymbol && equalGraphs(cx, ((NativeSymbol) s1).getKey(), ((NativeSymbol) s2).getKey());
		}
		return true;
	}
	...
	private static Object[] getIds(Context cx, final Scriptable s) {
	...
	private static Object getValue(Context cx, final Scriptable s, final Object id) {
		if (id instanceof Symbol) {
			return ScriptableObject.getProperty(cx, s, (Symbol) id);
		} else if (id instanceof Integer) {
			return ScriptableObject.getProperty(cx, s, (Integer) id);
		} else if (id instanceof String) {
			return ScriptableObject.getProperty(cx, s, (String) id);
		} else {
			throw new ClassCastException();
		}
	}
