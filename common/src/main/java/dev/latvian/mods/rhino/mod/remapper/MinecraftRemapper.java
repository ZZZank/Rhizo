...
public class MinecraftRemapper implements Remapper {
    ...
    private void load0(InputStream stream, boolean debug) throws Exception {
        ...
        RemappingHelper.LOGGER.info("Loading mappings for {}", MappingIO.readUtf(stream));

        int unmappedTypes = MappingIO.readVarInt(stream);
        var mappedTypes = new RemappedType[MappingIO.readVarInt(stream)];
        int arrayTypes = MappingIO.readVarInt(stream);
        ...
        for (int __ = 0; __ < unmappedTypes; __++) {
            int index = MappingIO.readVarInt(stream);
            var name = MappingIO.readUtf(stream);
            types[index] = new RemappedType(new RemappedClass(name, name, false), 0);
        }

        for (int i = 0; i < mappedTypes.length; i++) {
            int index = MappingIO.readVarInt(stream);
            var realName = MappingIO.readUtf(stream);
            var remappedName = MappingIO.readUtf(stream);
            types[index] = new RemappedType(new RemappedClass(realName.isEmpty() ? remappedName : realName,
                remappedName,
                true
            ), 0);
        ...
        for (int i = 0; i < arrayTypes; i++) {
            int index = MappingIO.readVarInt(stream);
            int type = MappingIO.readVarInt(stream);
            int array = MappingIO.readVarInt(stream);
        ...
        var sig = new String[MappingIO.readVarInt(stream)];

        for (int i = 0; i < sig.length; i++) {
            int params = MappingIO.readVarInt(stream);
            var sb = new StringBuilder();
            sb.append('(');

            for (int j = 0; j < params; j++) {
                sb.append(types[MappingIO.readVarInt(stream)].descriptorString());
            }
        ...
        for (var c : mappedTypes) {
            ...
            int fields = MappingIO.readVarInt(stream);
            int arg0 = MappingIO.readVarInt(stream);
            int argN = MappingIO.readVarInt(stream);

            for (int i = 0; i < fields; i++) {
                var unmappedName = MappingIO.readUtf(stream);
                var mmName = MappingIO.readUtf(stream);
    ...
    private static final class RemappedType {
        ...
        public String toString() {
            ...
            return parent.toString() + JavaPortingHelper.repeat("[]", array);
        }
        ...
        public String descriptorString() {
            if (descriptorString == null) {
                if (array > 0) {
                    descriptorString = JavaPortingHelper.repeat("[", array) + parent.descriptorString();
                } else {
