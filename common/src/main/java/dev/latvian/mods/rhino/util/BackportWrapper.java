...
/**
 * A copied version of {@code sun.invoke.util.Wrapper}
 */
public enum BackportWrapper {
    ...
    private BackportWrapper(Class<?> wtype, Class<?> ptype, char tchar, Object zero, Object emptyArray, int format) {
     ...
     */
    public boolean isConvertibleFrom(BackportWrapper source) {
     ...
     */
    public static BackportWrapper forPrimitiveType(Class<?> type) {
        BackportWrapper w = findPrimitiveType(type);
        if (w != null)  return w;
        if (type.isPrimitive())
    ...
    static BackportWrapper findPrimitiveType(Class<?> type) {
        BackportWrapper w = FROM_PRIM[hashPrim(type)];
     ...
     *  @throws IllegalArgumentException for unexpected types
     */
    public static BackportWrapper forWrapperType(Class<?> type) {
        BackportWrapper w = findWrapperType(type);
        if (w != null)  return w;
        for (BackportWrapper x : values())
    ...
    static BackportWrapper findWrapperType(Class<?> type) {
        BackportWrapper w = FROM_WRAP[hashWrap(type)];
     ...
     */
    public static BackportWrapper forBasicType(char type) {
        BackportWrapper w = FROM_CHAR[hashChar(type)];
     ...
     */
    public static BackportWrapper forBasicType(Class<?> type) {
        if (type.isPrimitive())
    ...
    private static final BackportWrapper[] FROM_PRIM = new BackportWrapper[16];
    private static final BackportWrapper[] FROM_WRAP = new BackportWrapper[16];
    private static final BackportWrapper[] FROM_CHAR = new BackportWrapper[16];
...
//    }
    ...
    private <T> T convert(Object x, Class<T> type, boolean isCast) {
        ...
        if (!isCast) {
            ...
            BackportWrapper source = findWrapperType(sourceType);
