...
class UnitParser {
	...
	public Unit readUnit() {
		...
		if (start == '$') {
			move();
			return VariableUnit.of(readW());
			// return VariableUnit.of(storage, readW());
		} else if (start == '-') {
			move();
			return readUnit().negate();
		} else if (start == '~' || start == '!') {
			move();
			return readUnit().bitNot();
		} else if (start == '#') {
			move();
			String hex = readW();
			...
			int b = i & 0xFF;
			int a = (i >> 24) & 0xFF;
			return ColorUnit.FACTORY.create(new Unit[]{ FixedUnit.of(i), FixedUnit.of(a) });
